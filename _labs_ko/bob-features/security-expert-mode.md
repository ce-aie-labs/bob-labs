...
생성되는 Mode, Skill, 파일의 이름과 개수는 Bob 버전과 실행 결과에 따라 위 화면과 다를 수 있다.
...
파일 개수보다 전체 코드 스캔, 규칙 검증, 보고서 작성이 각각 Skill로 분리되고 Custom Mode에서 순서대로 연결됐는지를 확인한다.
...
처음 접하는 사람을 위해 `.bob/`에서 사용하는 구성 요소를 간단히 정리하면 다음과 같다:

- **Mode**: Bob이 어떤 역할로 일할지, 어떤 지침을 따를지, 어떤 도구와 Skill을 사용할 수 있는지를 묶어 정의한 작업 환경이다. 이 실습의 Security Expert Mode는 보안 감사와 보고서 작성에 필요한 작업만 수행하도록 범위를 제한한다.
- **Skill**: 특정 작업을 반복해서 수행할 수 있도록 절차와 판단 기준을 문서로 저장한 재사용 가능한 작업 단위다. 코드 스캔, 규칙 검증, 보고서 작성처럼 큰 작업을 목적별로 나눌 수 있다.
- **MCP 서버**: Bob이 외부 시스템이나 도구와 연결될 수 있도록 표준화된 인터페이스를 제공한다. 예를 들어 이슈 관리 시스템, 데이터베이스, 사내 API의 정보가 필요할 때 Bob이 허용된 MCP 도구를 통해 읽거나 작업하도록 구성할 수 있다.
- **Rule**: 프로젝트에서 Bob이 항상 따라야 하는 공통 원칙과 제약을 정의한다. 코딩 표준, 보안 정책, 수정 금지 파일처럼 여러 Mode와 Skill에 일관되게 적용할 지침을 저장하는 데 사용한다.

Mode가 전체 작업의 역할과 범위를 정한다면, Skill은 그 안에서 실행할 절차를 제공하고, Rule은 항상 지켜야 할 기준을 제공한다. MCP 서버는 작업에 외부 정보나 기능이 필요할 때 연결 통로가 된다. 모든 프로젝트가 네 구성 요소를 전부 필요로 하는 것은 아니며, 이번 실행 결과에는 Mode와 Skill이 생성된다.
