...
- [x] **Event date and size** - **2026-09-01, 30-50 participants** (customers). The timeline above assumes a full ~6.5-hour day; the exact hours are confirmed closer to the date.
...
- [x] **Java Modernization assets** - in scope, **premium-gated**. Bob's Java Modernization is a paid **premium package** ([java-modernization-index](https://bob.ibm.com/docs/ide/premium-packages/java-modernization/java-modernization-index)), not part of base Bob - 4 workflows (Java upgrade, Liberty replatforming, UI modernization, unit test generation). We still lack access, so labs are drafted from the published workflow docs, marked premium-gated in the lab body, and carry a `Bob-verify` note until run against the real package. First lab: `migration/java-upgrade`.
